...
function Header({walletOn, showAccount, accountCarrier, disconnectwallet, open, accountModal, modalOpenstate, closeAccount, openAccount, connect, close}) {

    let history = useHistory();

    const [commonbal, setCommonBal] = useState("0");
	const [rarebal, setrarebal] = useState("0");
	const [legenbal, setlegendbal] = useState("0");
  ...
  return (
    <div className='header'>
        <div className='logoContainer'>
            <img src={logo} className="zomfilogo" alt="" />
            <div className='titles'>ZOMFI</div>
        </div>

        <div className='tags'>
            <div className='headerObjects'>
                <p className='myBalance' onClick={()=>history.push("/")}>HOME</p>
            </div>
            <div className='headerObje'>
                <p className='myBalance' onClick={()=>history.push("/mint/land")}>MINT NFT</p>
            </div>
            <div className='headerObje'>
                <p className='myBalance'>SELL NFT</p>
            </div>
        </div>
        ...
        <div className='btncarrier'>
            ...
            <AnimatePresence
                ...
                onExitComplete={()=> null}
            >
                {modalOpenstate && <Modal gcase={modalOpenstate} modalOpenstate={modalOpenstate} handleClose={close} connectfun={connect}/>}
            </AnimatePresence>
        </div>}
        {walletOn && 
        <div>
            <div className='walletconnected' onClick={()=>{
                accountModal ? closeAccount() : openAccount()
            }}>
                {showAccount}
            </div>
            <AnimatePresence
                ...
                onExitComplete={()=> null}
            >
                {accountModal && <WalletModal gcase={accountModal} modalOpenstate={accountModal} handleClose={closeAccount} account={accountCarrier} disconnect={disconnectwallet} commonbal={commonbal} rarebal={rarebal} legenbal={legenbal}/>}
            </AnimatePresence>
        </div>}
    </div>
  )
